...
public class BatchService {
    ...
    public List<String> getByJobID(String jobIDBch) {

        // List<BatchEntity> batch = batchRepo.findByJobIDBch(jobIDBch);
        // if (batch != null) {
        // return modelMapper.map(batch, new TypeToken<ArrayList<BatchDTO>>() {

        // }.getType());
        // } else {
        // return null;
        // }
        List<BatchEntity> batchList = batchRepo.findByJobIDBch(jobIDBch);
        List<String> batchIDs = new ArrayList<>();

        for (BatchEntity batch : batchList) {
            batchIDs.add(batch.getBatchID_regBch());
        }
